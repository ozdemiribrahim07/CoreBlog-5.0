...
$(document).ready(function () {
    const datatable = $('#blogtable').DataTable({
    ...
    $(document).on('click', '.btn-sil', function (event) {
        ...
        const blogname = tablerow.find('td:eq(1)').text();
        Swal.fire({
            title: 'Silmek istediğine emin misin?',
            text: `${blogname} geri getiremeyeceksin!`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (data) {
                        const blogsonuc = jQuery.parseJSON(data);
                        if (blogsonuc.SonucStatus === 1) {
                            Swal.fire(
                                'Silindi!',
                                'Blog silindi.',
                                'success'
